...
struct CIDisparityToDepth: View {
                                                    ...
                                                    if attributeClass == "CIImage" {
                                                        if inputKey == "inputImage" {
                                                            ...
                                                            HStack {
                                                                ...
                                                                GroupBox {
                                                                    VStack(alignment: .center) {
                                                                        Text("Disparity Image")
                                                                        
                                                                        Image(nsImage: disparityImage())
                                                                            .resizable()
                                                                            .aspectRatio(contentMode: .fit)
                                                                            .frame(width: 300, height: 300)
                                                                    }
...
//                        OutputCodeView(filter: $FilterViewModel.currentFilter)
    ...
    func applyFitler() -> NSImage? {
        ...
        guard let inputImage = FilterHelper().getDisparityCIImage() else { return nil }

        filter.setValue(inputImage, forKey: "inputImage")

        guard let outputImage = filter.outputImage else { return nil }
        
//        filterViewModel.currentFilter = filter
